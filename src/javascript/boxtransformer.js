...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Listen for the sexy portal button OR fallback to old button
    const triggerExpansion = () => {
        ...
        // CLEAR THE PORTAL FROM MAINBOX BEFORE BUILDING DASHBOARD
        mainBox.innerHTML = "";
...
${GLITCH("૮ ˶• ༝ •˶ ꒱ა ♡")}
        ...
        document.getElementById("glitchme")?.addEventListener("click", function () {
            ...
            this.textContent = "CREAM IS ETERNAL ♡⛧";
    ...
    const startExpansion = () => {
        // Remove the listener so it can't fire twice
        document.removeEventListener("enterPortal", startExpansion);
        triggerExpansion();
    };
    // This fires whether you click the portal OR the old button
    document.addEventListener("enterPortal", startExpansion);
    // Fallback for old button (in case buttons.js fails to load)
    const oldButton = document.getElementById("clickMeButton");
    if (oldButton) {
        oldButton.addEventListener("click", startExpansion);
    }
    // BONUS: Also allow Enter key on the portal
